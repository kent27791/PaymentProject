...
namespace Payment.Gateway.Extentions
{
    public static class GcoinExtentions
    {
        ...
        public static HttpResponseMessage SendGcoin<T>(string path, T obj)
        {
            ...
            string auth = SecurityHelper.GetBase64Encode($"{GCOIN_MERCHANT_NOPHONE}:{GCOIN_MERCHANT_NOAPI}");
            string nonce = ((int)DateTime.UtcNow.Subtract(new DateTime(1970, 1, 1)).TotalSeconds).ToString();
            string signature = SecurityHelper.GetHash_HMAC_SHA512(GCOIN_MERCHANT_SECRETKEY, $"{path}{queryString}{SecurityHelper.GetHash_SHA256(nonce)}");
